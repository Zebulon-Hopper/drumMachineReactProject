...
const handleSound=(event)=>{
  if(!event.code){
    ...
    let sound=require(`./Audio/${event.target.id}.wav`)
    let audio = new Audio(sound)
    audio.play() }
    else{
      console.log(event.code)
      setDisplay(event.code)
      let sound=require(`./Audio/${event.code}.wav`)
      let audio = new Audio(sound)
      audio.play() }
}
  return (
  <div id='drum-machine'>{/*User Story #1*/}
    <div id='display-container'>
      <h1 id='display'>{display}</h1>{/*User Story #2*/}
      </div>
      
  <button id='keyboard' onKeyPress={handleSound}>Click to use keyboard</button>
  <div id='drum-pad-container'>
  <button id='KeyQ'  className='drum-pad'  onClick={handleSound} >Q</button>
  <button id='KeyW'  className='drum-pad'  onClick={handleSound} >W</button>
  <button id='KeyE'  className='drum-pad'  onClick={handleSound} >E</button>
  <button id='KeyA'  className='drum-pad'  onClick={handleSound} >A</button>
  <button id='KeyS'  className='drum-pad'  onClick={handleSound} >S</button>
  <button id='KeyD'  className='drum-pad'  onClick={handleSound} >D</button>
  <button id='KeyZ'  className='drum-pad'  onClick={handleSound} >Z</button>
  <button id='KeyX'  className='drum-pad'  onClick={handleSound} >X</button>
  <button id='KeyC'  className='drum-pad'  onClick={handleSound} >C</button>
  </div>
  </div>
